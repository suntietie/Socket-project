...
#define HOSPITAL_LOCAL_PORT 32942
#define HOSPITAL_NAME "C"
#define HOSPITAL_LOCAL_PORT_STR "32942"
...
int main(int argc, char* argv[]) {
    ...
    // 1. Data parser
    Hospital hospitalC;
    hospitalC.fileParser();
    int fflag = hospitalC.setParam(location, capacity, occupancy, port);
    if (fflag == EXIT_FAILURE) {
        return EXIT_FAILURE;
    }
    
    // 2. UDP: send capacity and initial occupancy to scheduler
    int fd = hospitalC.create_bind_UDP();
    std::string init_msg;
    init_msg = hosp_port + " " + location + " " + std::to_string(capacity) + " " + std::to_string(occupancy);
    hospitalC.sendUDPdata(fd, init_msg, PORT_HOSPITAL);
    // loop in 3 - 6
    ...
    while(1) {
        ...
        std::pair<int, std::string> query = hospitalC.recvUDPData(fd);
        ...
        if (client_loc == hospitalC.location) {
            msg = hosp_port + " None None";
            std::cout << "Hospital " << hosp_name << " has capacity = " <<hospitalC.capacity<< ", occupation = "<<hospitalC.occupancy<<", availability= "<< hospitalC.calAvail() << std::endl;
        ...
        std::pair<int, std::string> decision = hospitalC.recvUDPData(fd);
        if (decision.second == "Chosen") {
            //update
            hospitalC.occupancy ++;
            std::cout << "Hospital " << hosp_name << " has been assigned to a client, occupation is updated to " << hospitalC.occupancy << ", availability is updated to " << hospitalC.calAvail() << std::endl;
        }
        std::cout << "\n----------------------------\n" << std::endl;
